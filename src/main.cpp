...
void loadScenes() {
  ...
  scenes["Ship"].models[1].model->m_materials[8].m_color =
      glm::vec3(0.380392, 0.588235, 0.266667);
...
void changeScene(std::string sceneName) {
  ...
  selected_material_index =
      scenes[currentScene].models[0].model->m_meshes[0].m_material_idx;
...
void initialize() {
  ...
  // Load shader program
  ///////////////////////////////////////////////////////////////////////////
  shaderProgram = labhelper::loadShaderProgram("./src/copyTexture.vert",
                                               "./src/copyTexture.frag");
  simpleShaderProgram =
      labhelper::loadShaderProgram("./src/simple.vert", "./src/simple.frag");
...
void display(void) {
  ...
  ///////////////////////////////////////////////////////////////////////////
  mat4 viewMatrix =
      lookAt(camera.position, camera.position + camera.direction, worldUp);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, pathtracer::rendered_image.width,
               pathtracer::rendered_image.height, 0, GL_RGB, GL_FLOAT,
               pathtracer::rendered_image.getPtr());
  ...
  if (showLightSources) {
    ...
    labhelper::setUniformSlow(simpleShaderProgram, "material_color",
                              pathtracer::point_light.color);
    ...
    for (int i = 0; i < pathtracer::disc_lights.size(); ++i) {
      ...
      mat4 modelMatrix = glm::translate(pathtracer::disc_lights[i].position) *
                         mat4(tbn) *
                         glm::scale(vec3(pathtracer::disc_lights[i].radius));
      glUseProgram(simpleShaderProgram);
      labhelper::setUniformSlow(simpleShaderProgram,
                                "modelViewProjectionMatrix",
                                projMatrix * viewMatrix * modelMatrix);
      labhelper::setUniformSlow(simpleShaderProgram, "material_color",
                                pathtracer::disc_lights[i].color);

      labhelper::debugDrawDisc();

      labhelper::debugDrawArrow(viewMatrix, projMatrix,
...
void gui() {
  ...
  ///////////////////////////////////////////////////////////////////////////
  labhelper::Model *selected_model =
  ...
  if (ImGui::CollapsingHeader("Models", ImGuiTreeNodeFlags_DefaultOpen)) {
    if (ImGui::Combo("Model", &selected_model_index, model_getter,
      ...
      selected_mesh_index = 0;
      selected_material_index =
          selected_model->m_meshes[selected_mesh_index].m_material_idx;
    }
    ...
    if (ImGui::Button("Save Materials")) {
      labhelper::saveModelMaterialsToMTL(
          selected_model, labhelper::file::change_extension(
                              selected_model->m_filename, ".mtl"));
    }
#endif
  }
  ...
  if (ImGui::CollapsingHeader("Light sources",
    ...
    ImGui::SliderFloat("Point light intensity multiplier",
    ...
    for (int i = 0; i < pathtracer::disc_lights.size(); ++i) {
      ...
      glm::vec2 dir(atan2(l.direction.z, l.direction.x) / (2 * M_PI) + 0.5,
                    acos(l.direction.y) / M_PI);
      ImGui::DragFloat2("Direction", &dir.x, 0.01, 0, 1);
      dir.x -= 0.5;
      dir.x *= 2 * M_PI;
      dir.y *= M_PI;
      l.direction =
          vec3(cos(dir.x) * sin(dir.y), cos(dir.y), sin(dir.x) * sin(dir.y));
