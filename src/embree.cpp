...
void addModel(const labhelper::Model *model, const mat4 &model_matrix) {
  ...
  //   2. rtcSetNewGeometryBuffer() — allocate + get pointer to vertex/index
  //   buffer
  //   3. Fill the buffers
  ...
  for (auto &mesh : model->m_meshes) {
    ...
    for (uint32_t i = 0; i < num_vertices; i++) {
      vec4 transformed =
          model_matrix * vec4(model->m_positions[mesh.m_start_index + i], 1.0f);
    ...
    // 3b. Fill triangle indices (sequential, since vertices are already
    // per-triangle)
    for (uint32_t i = 0; i < num_vertices; i++) {
      embree_tri_idxs[i] = i;
    }
...
Intersection getIntersection(const Ray &r) {
  ...
  // Detect entry vs exit before flipping: entering when ray and geometry
  // normal are anti-parallel (ray coming from outside).
  ...
  vec2 uv0 =
      model->m_texture_coordinates[((mesh->m_start_index / 3) + r.primID) * 3 +
                                   0];
  vec2 uv1 =
      model->m_texture_coordinates[((mesh->m_start_index / 3) + r.primID) * 3 +
                                   1];
