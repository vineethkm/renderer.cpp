...
SDL_Window *init_window_SDL(std::string caption, int width, int height) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      caption.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width,
...
GLuint loadCubeMap(const char *facePosX, const char *faceNegX,
                   const char *facePosY, const char *faceNegY,
  ...
  public:
    static void loadCubeMapFace(std::string filename, GLenum face) {
      int w, h, comp;
      unsigned char *image =
          stbi_load(filename.c_str(), &w, &h, &comp, STBI_rgb_alpha);
  ...
  // For cube maps, filtering across faces causes artifacts - so disable
  // filtering
  ...
  // GL_LINEAR_MIPMAP_LINEAR); // Use tri-linear mip map filtering
...
#if defined(_WIN32)
    std::stringstream ss;
    ss << file << "(" << line << "): GL Error #" << glErr << "(" << sError
       << ") " << std::endl;
...
void setupGLDebugMessages() {
  ...
  // Enable debug messages. Set a callback handle, and then enable all messages
  // to begin with.
  ...
  glDebugMessageControl(GL_DONT_CARE, GL_DONT_CARE, GL_DEBUG_SEVERITY_LOW, 0, 0,
                        false);
...
void fatal_error(std::string errorString, std::string title) {
  ...
  // On Win32 we'll use a message box. On !Win32, just print to stderr and
  // abort()
#if defined(_WIN32)
...
GLuint loadShaderProgram(const std::string &vertexShader,
                         const std::string &fragmentShader, bool allow_errors) {
  GLuint vShader = glCreateShader(GL_VERTEX_SHADER);
  GLuint fShader = glCreateShader(GL_FRAGMENT_SHADER);

  std::ifstream vs_file(vertexShader);
  std::string vs_src((std::istreambuf_iterator<char>(vs_file)),
                     std::istreambuf_iterator<char>());

  std::ifstream fs_file(fragmentShader);
  std::string fs_src((std::istreambuf_iterator<char>(fs_file)),
                     std::istreambuf_iterator<char>());
...
GLuint createAddAttribBuffer(GLuint vertexArrayObject, const void *data,
                             const size_t dataSize, GLuint attributeIndex,
                             GLsizei attributeSize, GLenum type,
                             GLenum bufferUsage) {
  GLuint buffer = 0;
...
GLuint createAddIndexBuffer(GLuint vertexArrayObject, const void *data,
                            const size_t dataSize, GLenum bufferUsage) {
  glBindVertexArray(vertexArrayObject);
  GLuint buffer = 0;
  glGenBuffers(1, &buffer);
...
void setUniformSlow(GLuint shaderProgram, const char *name,
                    const glm::mat4 &matrix) {
  glUniformMatrix4fv(glGetUniformLocation(shaderProgram, name), 1, false,
                     &matrix[0].x);
}
...
void setUniformSlow(GLuint shaderProgram, const char *name,
                    const uint32_t nof_values, const glm::vec3 *values) {
  glUniform3fv(glGetUniformLocation(shaderProgram, name), nof_values,
               (float *)values);
}

void debugDrawArrow(const glm::mat4 &viewMat, const glm::mat4 &projMat,
                    glm::vec3 start, glm::vec3 point) {
  using namespace glm;
  ...
  if (vao == 0) {
    ...
    labhelper::createAddAttribBuffer(vao, positions.data(),
                                     positions.size() * sizeof(glm::vec3), 0, 3,
                                     GL_FLOAT);
...
void debugDrawSphere() {
  ...
  if (vao == 0) {
    ...
    labhelper::createAddAttribBuffer(vao, positions.data(),
                                     positions.size() * sizeof(glm::vec3), 0, 3,
                                     GL_FLOAT);
    labhelper::createAddIndexBuffer(vao, indices.data(),
...
void saveScreenshot() {
  ...
  stbi_write_png(fname.str().c_str(), lwidth, lheight, n_channels, img.data(),
                 0);
}
...
void drawFullScreenQuad() {
  ...
  if (vertexArrayObject == 0) {
    ...
    static const glm::vec2 positions[] = {{-1.0f, -1.0f}, {1.0f, -1.0f},
                                          {1.0f, 1.0f},   {-1.0f, -1.0f},
                                          {1.0f, 1.0f},   {-1.0f, 1.0f}};
    labhelper::createAddAttribBuffer(
        vertexArrayObject, positions,
...
std::string change_extension(const std::string &file_name,
